...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const messages = [
      ...
      { role: 'assistant', content: 'Understood. I\'m ready to help improve your resume. What would you like to work on?' },
      ...(history ?? []).map(({ role, content }: { role: string; content: string }) => ({ role, content })),
