...
export default function Minimal({ resume, isPro, showHeader = true }: TemplateProps) {
  ...
  const S = {
    ...
    entryMeta: { fontSize: s.fontSize - 1, color: '#888888', whiteSpace: 'nowrap' as const, flexShrink: 0, marginLeft: 8 },
    ...
    bulletDash: { fontSize: s.fontSize, color: '#AAAAAA', marginRight: 5, lineHeight: s.lineHeight, flexShrink: 0 },
    bulletText: { fontSize: s.fontSize, color: '#333333', flex: 1, lineHeight: s.lineHeight },
    textPara: { fontSize: s.fontSize, color: '#444444', marginBottom: 4, lineHeight: 1.6 },
    kvRow: { display: 'flex', flexDirection: 'row' as const, marginBottom: 3, gap: 6 },
    kvKey: { fontSize: s.fontSize - 1, color: '#888888', width: 60, flexShrink: 0 },
...
function SectionBlock({ section, S }: { section: ResumeSection; S: MinimalStyles }) {
  if (section.items.length === 0) return null
  return (
    <section data-section={section.id} style={S.section}>
      <h2 style={S.sectionTitle}>{section.title}</h2>
      {section.items.map((item, i) => (
        <ItemBlock key={i} item={item} isKeyValueSection={section.hint === 'keyvalue'} S={S} />
      ))}
    </section>
  )
}

function ItemBlock({ item, isKeyValueSection, S }: { item: SectionItem; isKeyValueSection: boolean; S: MinimalStyles }) {
  switch (item.kind) {
    case 'keyvalue': {
